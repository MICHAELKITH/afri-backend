...
func getJWTSecret() []byte {
	...
	if secret == "" {
		log.Println("[WARN] JWT_SECRET is not set. Falling back to a temporary unsafe secret.")
...
func SignUp(c *fiber.Ctx) error {
	...
	// Crucial Fix: Enforce lowercase emails on creation to prevent login/verification desyncs
...
func VerifyEmail(c *fiber.Ctx) error {
	...
	if err := database.DB.Where(
		"email = ? AND email_verify_expires > ?",
		body.Email,
		time.Now(),
	...
	// Crucial Fix: Explicitly select columns to force zero-value strings and nil values into DB
...
func ResetPassword(c *fiber.Ctx) error {
	...
	// Crucial Fix: Explicitly select columns to force empty strings and nil values back into GORM
...
func sendVerifyEmail(toEmail, firstName, otp string) {
	...
	htmlBody := buildVerifyEmailHTML(firstName, otp, "AfCFTApreneurship Arena")
	...
	if err != nil {
		log.Printf("[ERROR] Failed to send verify email to %s via Resend: %v\n", toEmail, err)
		return
	}
	log.Printf("[INFO] Verify email sent to %s via Resend. ID: %s\n", toEmail, sent.Id)
}
...
func sendOTPEmail(toEmail, firstName, otp string) {
	...
	client := resend.NewClient(apiKey)
	fromEmail := "Arena Support <noreply@traders.kazini.africa>"
	subject := "Your Password Reset Code — AfCFTApreneurship Arena"
	htmlBody := buildOTPEmailHTML(firstName, otp, "AfCFTApreneurship Arena")
	...
	sent, err := client.Emails.Send(params)
	if err != nil {
		log.Printf("[ERROR] Failed to send OTP email to %s via Resend: %v\n", toEmail, err)
		return
	}
	log.Printf("[INFO] OTP email sent to %s via Resend. ID: %s\n", toEmail, sent.Id)
}

// ─── Template Builders (Fixes Applied for Escape Sequences) ────────────────────
...
func buildVerifyEmailHTML(firstName, otp, appName string) string {
	...
	// Crucial Fix: Escaped all structural and table CSS percentage verbs using '%%' instead of '%'
        ...
        <tr>
          <td style="padding:36px 40px;">
            ...
            <p style="margin:0 0 28px;font-size:14px;color:#475569;line-height:1.7;">
              Welcome to the Arena! Enter this 6-digit code to verify your email address.
