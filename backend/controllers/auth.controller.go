...
func getJWTSecret() []byte {
	...
	if secret == "" {
		log.Println("[WARN] JWT_SECRET is not set.")
		return []byte("temporary_unsafe_secret_change_me")
	}
	return []byte(secret)
}
...
func sendVerifyEmail(toEmail, firstName, otp string) {
	...
	if apiKey == "" {
		log.Println("[WARN] RESEND_API_KEY not configured. Verify OTP:", otp)
		return
	}
	...
	if err != nil {
		log.Printf("[ERROR] Failed to send verify email to %s: %v\n", toEmail, err)
		return
	}
	log.Printf("[INFO] Verify email sent to %s. ID: %s\n", toEmail, sent.Id)
}
...
func sendOTPEmail(toEmail, firstName, otp string) {
	apiKey := os.Getenv("RESEND_API_KEY")
	if apiKey == "" {
		log.Println("[WARN] RESEND_API_KEY not configured. OTP:", otp)
		return
	}
	...
	params := &resend.SendEmailRequest{
		...
		To:      []string{toEmail},
		Subject: "Your Password Reset Code — AfCFTApreneurship Arena",
		Html:    buildOTPEmailHTML(firstName, otp, "AfCFTApreneurship Arena"),
	}
	...
	log.Printf("[INFO] OTP email sent to %s. ID: %s\n", toEmail, sent.Id)
}
...
func VerifyEmail(c *fiber.Ctx) error {
	...
	if err := database.DB.Model(&user).Select("EmailVerified", "EmailVerifyToken", "EmailVerifyExpires").Updates(map[string]interface{}{
		"email_verified":       true,
		"email_verify_token":   "",
...
func Login(c *fiber.Ctx) error {
	...
	var user models.User
	if err := database.DB.Where("email = ?", strings.ToLower(strings.TrimSpace(body.Email))).First(&user).Error; err != nil {
	...
	if !user.EmailVerified {
		return c.Status(403).JSON(fiber.Map{"error": "Please verify your email before logging in.", "unverified": true, "email": user.Email})
	}
	signed, err := issueToken(user)
	if err != nil {
...
func ResetPassword(c *fiber.Ctx) error {
	...
	if err := database.DB.Model(&user).Select("Password", "PasswordResetToken", "PasswordResetExpires").Updates(map[string]interface{}{
...
// RULE: Use %% only for CSS percentage signs (width:100%%).
//       Use %s / %d for fmt.Sprintf substitutions.

func buildVerifyEmailHTML(firstName, otp, appName string) string {
	digits := strings.Split(otp, "")
	var digitBoxes string
	for _, d := range digits {
		digitBoxes += fmt.Sprintf(`<td style="padding:0 4px;"><div style="width:44px;height:54px;border:2px solid #d1fae5;border-radius:10px;background:#f0fdf4;font-size:28px;font-weight:700;color:#047857;font-family:'Segoe UI',Arial,sans-serif;line-height:54px;text-align:center;">%s</div></td>`, d)
	}
        ...
        <tr>
          <td style="padding:36px 40px;">
            ...
            <p style="margin:0 0 28px;font-size:14px;color:#475569;line-height:1.7;">
              Welcome to the Arena! Enter this 6-digit code to verify your email.
              ...
              <tr>
                <td style="background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:12px 16px;">
                  <p style="margin:0;font-size:12.5px;color:#92400e;">
                    ⚠ <strong>Didn't create an account?</strong> Safely ignore this email. Never share this code.
                  </p>
                </td>
              </tr>
            </table>
...
func buildOTPEmailHTML(firstName, otp, appName string) string {
	...
	return fmt.Sprintf(`<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"/><title>Your Reset Code</title></head>
<body style="margin:0;padding:0;background:#f0fdf4;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;padding:40px 20px;">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;border:1px solid #d1fae5;overflow:hidden;max-width:100%%;">
          ...
          <td style="padding:36px 40px;">
            ...
            <table width="100%%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:12px 16px;">
                  <p style="margin:0;font-size:12.5px;color:#92400e;">
                    ⚠ <strong>Didn't request this?</strong> Ignore this email — your password will not change. Never share this code.
                  </p>
                </td>
              </tr>
            </table>
